...
fn priority_ordering() {
    // Human > Normal, System > Human
    assert!(Priority::Human > Priority::Normal);
    assert!(Priority::System > Priority::Human);
}

#[test]
fn queue_event_priority_human_message() {
    let event = QueueEvent::HumanMessage {
        session: SessionKey::new("s1"),
        content: "hi".into(),
        priority: Priority::Human,
    };
    assert_eq!(event.priority(), Priority::Human);
}

#[test]
fn queue_event_priority_tool_result() {
    let event = QueueEvent::ToolResult {
        session: SessionKey::new("s1"),
        tool_use_id: "t1".into(),
        name: "read".into(),
        result: "ok".into(),
...
#[test]
fn queue_event_priority_shutdown() {
    let event = QueueEvent::Shutdown;
    assert_eq!(event.priority(), Priority::System);
}

// ===========================================================================
// CancellationToken integration with run_turn_cancellable
// ===========================================================================

#[tokio::test]
async fn run_turn_cancellable_immediate_cancel() {
    // If cancellation token is already cancelled before run_turn_cancellable,
    // it should return quickly with a "cancelled" Done event.
    let api_key = match load_api_key() {
        Some(k) => k,
        None => {
            eprintln!("SKIP: no ANTHROPIC_API_KEY");
    ...
    let tools = agenticlaw_tools::ToolRegistry::new();
    let config = AgentConfig {
        default_model: "claude-haiku-4-5-20251001".into(),
        max_tool_iterations: 5,
    ...
    let session_key = SessionKey::new("test-cancel");
    let (event_tx, mut event_rx) = tokio::sync::mpsc::channel(256);
    ...
    let result = runtime
        .run_turn_cancellable(&session_key, "ping", event_tx, cancel)
        .await;
    assert!(result.is_ok());

    // Should get a Done with "cancelled" stop_reason
    let mut got_cancelled = false;
    while let Some(event) = event_rx.recv().await {
        if let AgentEvent::Done { stop_reason } = event {
    ...
    assert!(got_cancelled, "Expected cancelled Done event");

    // The user message should still be in the session
    let session = runtime.sessions().get(&session_key).unwrap();
    assert_eq!(session.message_count().await, 1); // user message only
}

#[tokio::test]
async fn run_turn_cancellable_cancel_during_stream() {
    // Cancel while the LLM is streaming — should stop cleanly
    let api_key = match load_api_key() {
        Some(k) => k,
        None => {
            eprintln!("SKIP: no ANTHROPIC_API_KEY");
    ...
    let session_key = SessionKey::new("test-cancel-stream");
    let (event_tx, mut event_rx) = tokio::sync::mpsc::channel(256);
    ...
    let handle = tokio::spawn(async move {
        runtime
            .run_turn_cancellable(&session_key, "go", event_tx, cancel_clone)
            .await
    });

    // Wait for some text to arrive, then cancel
    ...
    loop {
        match tokio::time::timeout(std::time::Duration::from_secs(10), event_rx.recv()).await {
            Ok(Some(AgentEvent::Text(_))) => {
                if !got_text {
                    got_text = true;
                    // Cancel after first text
                    cancel.cancel();
                }
            }
            Ok(Some(AgentEvent::Done { stop_reason: sr })) => {
                got_done = true;
                stop_reason = sr;
                break;
            }
    ...
    assert!(
        got_text,
        "Should have received some text before cancellation"
    );
    assert!(got_done, "Should have received Done event");
    assert_eq!(
        stop_reason, "cancelled",
        "Stop reason should be 'cancelled'"
    );
}
