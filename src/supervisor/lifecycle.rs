...
pub async fn spawn_worker(name: &str, card: Option<&str>, briefing: Option<&str>) -> Result<(), String> {
    ...
    log::info("worker_spawned", serde_json::json!({
...
pub async fn harvest_worker(name: &str, base: &str) -> Result<(), String> {
    ...
    log::info("worker_harvested", serde_json::json!({
...
pub async fn handle_failure(failure: &FailureMode, dry_run: bool) {
    match failure {
        FailureMode::Frozen { session, unchanged_secs } => {
            log::warn("failure_detected", serde_json::json!({
                "type": "frozen", "session": session, "unchanged_secs": unchanged_secs
            }));
            if !dry_run {
                let _ = tmux::send_keys(session, "# supervisor: you appear frozen, please continue").await;
            }
        }
        FailureMode::Deranged { session, repeated_pattern, count } => {
            log::warn("failure_detected", serde_json::json!({
        ...
        FailureMode::InfiniteLoop { session, operation, retries } => {
            log::warn("failure_detected", serde_json::json!({
        ...
        FailureMode::RabbitHoling { session, cycles_off_frontier } => {
            log::warn("failure_detected", serde_json::json!({
