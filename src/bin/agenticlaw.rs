    ...
    match cli.command {
        Some(Commands::Gateway { port, bind, token, no_auth, workspace, system_prompt }) => {
            ...
            let config = ExtendedConfig {
                gateway: GatewayConfig { port, bind: bind_mode, auth },
        ...
        Some(Commands::Chat { workspace, session, model, resume, embedded, port }) => {
            let workspace = workspace.or_else(|| Some(resolve_home(None, &oc)));
            let model = model.or_else(|| oc.default_model());
            let session_name = session.unwrap_or_else(|| uuid::Uuid::new_v4().to_string()[..8].to_string());

            if !embedded {
                if let Ok(health) = agenticlaw_gateway::service::check_health(port).await {
                    let version = health["version"].as_str().unwrap_or("?");
                    eprintln!("Connecting to agenticlaw gateway v{} on port {}...", version, port);

                    let token = std::env::var("RUSTCLAW_GATEWAY_TOKEN")
                        .or_else(|_| std::env::var("OPENCLAW_GATEWAY_TOKEN"))
                        .ok();

                    return agenticlaw_gateway::tui_client::run_tui_client(port, session_name, token).await;
                }
                eprintln!("No running gateway on port {}. Falling back to embedded mode.", port);
        ...
        Some(Commands::Status { port }) => {
            match agenticlaw_gateway::service::check_health(port).await {
                Ok(health) => {
                    ...
                    println!("  Tools:    {}", tools);
                    if let Some(l) = layer { println!("  Layer:    {}", l); }

                    if let Ok(resp) = reqwest::get(format!("http://127.0.0.1:{}/surface", port)).await {
                        if let Ok(surface) = resp.json::<serde_json::Value>().await {
                            let provides = surface["provides"].as_array().map(|a| a.len()).unwrap_or(0);
...
async fn start_conscious(cli: &Cli, oc: &OpenclawConfig) -> anyhow::Result<()> {
    ...
    let souls = cli.souls.as_ref().map(|s| expand_tilde(s)).unwrap_or_else(|| {
        let candidates = [
    ...
    let config_path = cli.config.as_ref().map(|p| expand_tilde(p))
...
async fn start_gateway_only(cli: &Cli, oc: &OpenclawConfig) -> anyhow::Result<()> {
    ...
    let config = ExtendedConfig {
        gateway: GatewayConfig { port: resolve_port(cli.port), bind: bind_mode, auth },
        anthropic_api_key: std::env::var("ANTHROPIC_API_KEY").ok(),
        workspace_root,
        system_prompt: merged_prompt,
    };
...
fn resolve_home(explicit: Option<&PathBuf>, oc: &OpenclawConfig) -> PathBuf {
    ...
    if let Ok(env_ws) = std::env::var("AGENTICLAW_WORKSPACE") { return PathBuf::from(env_ws); }
    if let Ok(env_ws) = std::env::var("RUSTCLAW_WORKSPACE") { return PathBuf::from(env_ws); }
...
fn resolve_port(requested: u16) -> u16 {
    if !port_in_use(requested) { return requested; }
    for p in [18799u16] { if !port_in_use(p) { return p; } }
    for p in 18800..=18899 { if !port_in_use(p) { return p; } }
    requested
}
